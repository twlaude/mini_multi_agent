...
def upload_image_describe(filename: str, content: bytes, content_type: str, question: str) -> Any:
    try:
        response = httpx.post(
            f"{BACKEND_URL}/api/media/image-describe",
            files={"image": (filename, content, content_type)},
            data={"question": question},
            timeout=REQUEST_TIMEOUT,
        )
...
def upload_audio_stt(filename: str, content: bytes, content_type: str) -> Any:
    try:
        response = httpx.post(
            f"{BACKEND_URL}/api/media/stt",
