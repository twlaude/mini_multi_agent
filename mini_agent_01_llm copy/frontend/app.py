...
camera_page = st.Page("app_pages/09_camera_voice_guide.py", title="카메라 음성 가이드")

navigation = st.navigation(
    [home_page, concept_page, travel_page, environment_page, llm_page, compare_page, image_page, tts_page, camera_page],
    position="hidden",
)
...
with st.sidebar:
    ...
    with st.expander("01. LLM에서 Agent로", expanded=True):
        ...
        st.page_link(camera_page, label="1-7. 카메라 음성 가이드")
