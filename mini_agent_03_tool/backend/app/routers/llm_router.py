...
llm_router = APIRouter(tags=["01 · LLM 기초"])
...
@llm_router.get("/api/providers")
def providers() -> dict:
    return {"default_provider": settings.llm_provider, "providers": provider_status()}


@llm_router.post("/api/concepts/compare", response_model=ConceptCompareResult)
...
@llm_router.post("/api/travel/classify", response_model=TravelIntentResult)
...
@llm_router.post("/api/generate", response_model=GenerateResult)
...
@llm_router.post("/api/providers/compare", response_model=ProviderCompareResult)
